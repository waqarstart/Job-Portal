...
import { safeOriginalName, escapeRegex } from "../utils/security.js";
...
// SVG is deliberately excluded: SVGs can carry embedded <script> and are a
// well-known stored-XSS vector when later opened directly or embedded via
// <object>/<iframe>. Raster formats only.
...
const companyMediaStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) =>
    cb(null, `${Date.now()}-${Math.round(Math.random() * 1e9)}${path.extname(safeOriginalName(file.originalname))}`),
});
...
router.get("/dashboard", requireAuth, requireHR, async (req, res) => {
  try {
    ...
    const totalJobViews = myJobs.reduce((total, job) => total + (job.views || 0), 0);
...
router.get("/jobs", requireAuth, requireHR, async (req, res) => {
  try {
    ...
    if (search) baseFilter.title = { $regex: escapeRegex(search), $options: "i" };
