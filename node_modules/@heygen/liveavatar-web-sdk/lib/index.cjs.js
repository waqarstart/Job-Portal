...
exports.ElevenLabsAgentCommandType = void 0;
...
})(exports.ElevenLabsAgentCommandType || (exports.ElevenLabsAgentCommandType = {}));
...
function parseAgentTypeFromToken(token) {
    const decoded = decodeSessionTokenPayload(token);
    if (!decoded)
        return exports.AgentType.UNKNOWN;
    ...
    if (typeof data.agent_type === "string") {
        ...
        if (explicit in exports.AgentType)
            return explicit;
    }
    if (data.elevenlabs_agent_config)
        return exports.AgentType.ELEVENLABS_AGENT;
    if (data.openai_realtime_config)
        return exports.AgentType.OPENAI_REALTIME;
    if (data.gemini_realtime_config)
        return exports.AgentType.GEMINI_REALTIME;
    return exports.AgentType.FULL;
}
class LiveAvatarSession extends events.EventEmitter {
    ...
    publishAgentControl(payload) {
        if (this.room.state !== "connected") {
            throw new Error("LiveKit room not connected — cannot publish agent-control payload");
        }
...
class ElevenLabsAgentSessionError extends Error {
    constructor(message) {
        super(message);
 ...
 */
class ElevenLabsAgentSession extends LiveAvatarSession {
    constructor(sessionAccessToken, config) {
    ...
    sendElevenLabsAgentCommand(payload, options) {
        var _a;
        if (!this.assertConnected()) {
            throw new ElevenLabsAgentSessionError("Session must be connected before sending ElevenLabs agent commands");
        }
        const sessionId = this.sessionId;
        if (!sessionId) {
            throw new ElevenLabsAgentSessionError("Session id is not yet available — wait for session.started");
        }
        const event_id = this.generateEventId();
        const wrapper = Object.assign({ event_type: exports.CommandEventsEnum.ELEVENLABS_AGENT_COMMAND, event_id, session_id: sessionId, source_event_id: (_a = options === null || options === void 0 ? void 0 : options.sourceEventId) !== null && _a !== void 0 ? _a : null }, payload);
    ...
    sendUserMessage(text) {
        return this.sendElevenLabsAgentCommand({
            elevenlabs_event_type: exports.ElevenLabsAgentCommandType.USER_MESSAGE,
            data: { text },
        });
    }
    ...
    sendContextualUpdate(text) {
        return this.sendElevenLabsAgentCommand({
            elevenlabs_event_type: exports.ElevenLabsAgentCommandType.CONTEXTUAL_UPDATE,
            data: { text },
        });
    }
    ...
    sendUserActivity() {
        return this.sendElevenLabsAgentCommand({
            elevenlabs_event_type: exports.ElevenLabsAgentCommandType.USER_ACTIVITY,
        });
    }
    ...
    sendClientToolResult(args) {
        var _a;
        return this.sendElevenLabsAgentCommand({
            elevenlabs_event_type: exports.ElevenLabsAgentCommandType.CLIENT_TOOL_RESULT,
            data: {
...
exports.ElevenLabsAgentSession = ElevenLabsAgentSession;
exports.ElevenLabsAgentSessionError = ElevenLabsAgentSessionError;
exports.LiveAvatarSession = LiveAvatarSession;
exports.parseAgentTypeFromToken = parseAgentTypeFromToken;
