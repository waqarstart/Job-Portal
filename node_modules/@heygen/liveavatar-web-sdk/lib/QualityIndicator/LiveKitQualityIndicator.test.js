...
import { ConnectionState, Room, ConnectionQuality as LiveKitConnectionQuality, } from "livekit-client";
...
describe("LiveKitConnectionQualityIndicator", () => {
    for (const state of [
        ...
        it(`resolves to BAD when connection state is ${state}`, () => {
            ...
            const room = new Room();
            const indicator = new LiveKitConnectionQualityIndicator(onConnectionQualityChanged);
    ...
    it("resolves to GOOD when connection state is connected", () => {
        ...
        const indicator = new LiveKitConnectionQualityIndicator(onConnectionQualityChanged);
        indicator.start(room);
        testContext.roomInstance._triggerConnectionStateChanged(ConnectionState.Connected);
        expect(onConnectionQualityChanged).toHaveBeenCalledWith(ConnectionQuality.GOOD);
    });
});
