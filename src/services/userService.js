...
export async function uploadProfileDocument(type, file) {
  const formData = new FormData();
  formData.append("document", file);
  const { data } = await api.post(`/users/me/documents/${type}`, formData, {
    headers: { "Content-Type": "multipart/form-data" },
  });
  return data;
}

export async function deleteProfileDocument(type) {
  const { data } = await api.delete(`/users/me/documents/${type}`);
  return data;
}

export async function uploadOtherDocument(name, file) {
  const formData = new FormData();
  formData.append("name", name);
  formData.append("document", file);
  const { data } = await api.post("/users/me/documents/other", formData, {
    headers: { "Content-Type": "multipart/form-data" },
  });
  return data;
}

export async function deleteOtherDocument(id) {
  const { data } = await api.delete(`/users/me/documents/other/${id}`);
  return data;
}
