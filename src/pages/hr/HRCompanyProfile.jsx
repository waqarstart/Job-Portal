...
export default function HRCompanyProfile() {
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      showToast("Company profile is saved.");
    } catch (err) {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.6fr_1fr] gap-6 items-start">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <button type="submit" disabled={saving}
            className="transition-all duration-200 w-full rounded-lg bg-blue-600 py-3 font-semibold text-white hover:bg-blue-700 disabled:opacity-60 xl:hidden">
          ...
          <div className="absolute left-0 right-0 top-full z-50 mt-1 max-h-56 overflow-y-auto rounded-xl border border-gray-100 bg-white py-1 shadow-xl">
            {options.map((opt) => (
              <button
                ...
                className={`flex w-full items-center justify-between px-4 py-2 text-sm transition-[background-color,padding-left] duration-200 ease hover:bg-blue-50 hover:pl-5 hover:text-blue-600 ${
