...
export default function SavedJobs() {
  ...
  // Close menus on outside click via a passive listener — not a full-screen
  // overlay, so the rest of the page stays clickable while a menu is open.
  useEffect(() => {
    function handler(e) {
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div ref={filtersRef} className="relative">
            <button
              ...
              className={`flex h-11 w-11 items-center justify-center rounded-xl border bg-white transition-all duration-200 hover:bg-gray-50 ${
                ...
                <div className="absolute right-0 top-full z-20 mt-1 w-52 rounded-xl border border-gray-100 bg-white p-3 shadow-xl">
                  ...
                  <div className="space-y-1">
                    <button
                      ...
                      className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm transition-[background-color,padding-left] duration-200 ease hover:bg-blue-50 hover:pl-4 hover:text-blue-600 ${
          ...
          return (
            <div key={_id} className="rounded-2xl border bg-white p-5 shadow-sm relative">
                    ...
                    {openMenuId === _id && (
                        <div className="absolute right-0 top-full mt-1 w-40 rounded-xl border border-gray-100 bg-white shadow-xl z-20 py-1">
                          <Link
                            ...
                            className="block px-4 py-2 text-sm text-gray-700 transition-[background-color,padding-left] duration-200 ease hover:bg-blue-50 hover:pl-5 hover:text-blue-600"
                          >
                            View Job
                          </Link>
                          <button
                            onClick={() => handleUnsave(job._id)}
                            className="block w-full text-left px-4 py-2 text-sm text-red-600 transition-[background-color,padding-left] duration-200 ease hover:bg-red-50 hover:pl-5"
                          >
                            Remove
                          </button>
                        </div>
              ...
              <div className="mt-4 pt-4 border-t flex items-center justify-end gap-3">
                <button
                  onClick={() => handleUnsave(job._id)}
                  className="transition-all duration-200 flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50"
                >
