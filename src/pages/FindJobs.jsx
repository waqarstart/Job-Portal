...
function JobPanel({ job, saved, onSave, onApply, onClose, isLoggedIn }) {
  if (!job) return (
...
export default function FindJobs() {
  ...
  useEffect(() => {
    ...
    // Category cards on the home page link here with ?category=<name> —
    // pre-select that category in the existing (already-working) client-side
    // category filter, instead of trying to match it against job titles.
    setFCategories(cat ? [cat] : []);
    loadJobs(t, c);
        ...
        <div className="w-full px-4 sm:px-6 py-3">
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="flex flex-1 items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2 focus-within:border-blue-400 transition">
              ...
              {titleQ && <button type="button" onClick={() => setTitleQ("")}><HiOutlineXMark className="h-4 w-4 text-gray-400 transition-all duration-200 hover:opacity-75" /></button>}
            </div>
          ...
          <div className="rounded-2xl border border-gray-100 bg-white shadow-sm flex flex-col h-full overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 shrink-0">
              ...
              <button
                ...
                className="transition-all duration-200 flex items-center gap-1 text-xs font-medium text-blue-600 hover:underline"
              >
        ...
        <div className="flex-1 min-w-0 flex flex-col sticky top-4" style={{ height: colHeight }}>
          ...
          <div className="flex items-center justify-between mb-3 shrink-0">
            <div>
              ...
              {!titleQ && fCategories.length > 0 && (
                <p className="text-xs text-blue-600 mt-0.5">in {fCategories.join(", ")}</p>
              )}
            </div>
